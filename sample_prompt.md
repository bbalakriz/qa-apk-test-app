Generate a Gherkin feature file and corresponding step definitions for the PassengerDetailsScreen in the MobileAppCheckIn React Native app.
Use the combination of React Native, Appium, and WebdriverIO (WDIO).
The feature should follow the same structure and implementation conventions as the working CheckInScreen example provided below.

In the PassengerDetailsScreen, the user should be able to:
- view and verify their name and PNR
- select their seat
- continue to the next screen

Assume that:

- Test IDs are not working in this APK.
- Selectors should use fallback strategies like text, placeholder, or XPath.
- Use the following working example for CheckInScreen as a reference for formatting, selector strategies, and fallback handling.
